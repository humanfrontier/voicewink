...
final class LogExporter {
    ...
    func exportLogs() async throws -> URL {
        ...
        logger.notice("🎙️ Log export completed, \(AppLogRedaction.fileSummary(fileURL), privacy: .public)")
        return fileURL
    }
    ...
    private func fetchLogs() async throws -> [String] {
        ...
        logLines.append("=== VoiceWink Diagnostic Logs ===")
    ...
    private func saveLogsToFile(_ logs: [String]) throws -> URL {
        ...
        let fileName = "VoiceWink_Logs_\(timestamp).log"
